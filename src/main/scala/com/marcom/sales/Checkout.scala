...
class Checkout extends Requirements {
  ...
  def price(items: List[String]): Int Or Every[ErrorMessage] = {
    items match {
  ...
  def priceItems(items: List[String]): Int Or Every[ErrorMessage] = {
    val m = items.groupBy(i => i).mapValues(_.size)
    m.foldLeft[Int Or Every[ErrorMessage]](Good(0)) { (i, t) => i match {
      case Bad(msgs) => t match {
        case (null, c) => Bad(msgs ++ One(s"null is not a valid item"))
        case ("apple", c) => Bad(msgs)
        case ("orange", c) => Bad(msgs)
        case (str, c) => Bad(msgs ++ One(s"unknown fruit $str"))
      }
